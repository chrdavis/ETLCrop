...
| [`ETWCropInstaller`](ETWCropInstaller) | `ETWCropInstaller.msi` | WiX installer for the desktop app. |
...
[`ETWCropInstaller`](ETWCropInstaller) project. It installs `ETWCrop.App.exe` and its
dependencies under *Program Files*, adds a Start Menu shortcut, and registers the app in
Add/Remove Programs with the ETWCrop icon.

```pwsh
dotnet build ETWCropInstaller\ETWCropInstaller.wixproj -c Release
```

The MSI is written to `ETWCropInstaller\bin\Release\ETWCropInstaller.msi`. Building the MSI
...
The application icon ([`ETLCrop.ico`](ETWCrop.App/ETLCrop.ico)) and the installer banner/
